...
// 是不同的轴：构建 locale 变了，数据语言不随之变。lang 不在此列时回退 en 数据。
export const SUPPORTED_LANGUAGES = ["en", "zh-Hans", "zh-Hant", "ja", "ko", "de", "fr", "es", "it", "pt", "ru", "ar", "hi", "bn", "vi", "th", "tr", "ind"];
 ...
 * - 根据 ID 过滤返回
 */
export async function fetchCardsByIds(ids: number[], lang: string = "en"): Promise<CardData[]> {
  if (!ids || ids.length === 0) {
    return [];
  }
 ...
 * - 按 ALL_IDS 顺序返回
 */
export async function fetchNextCards(excludeIds: number[], batchSize: number = 8, lang: string = "en"): Promise<CardData[]> {
 ...
 * - 根据 tags 和关键词过滤
 */
export async function searchCardsLocally(tags: string[], searchName: string | null, lang: string = "en", operator: "AND" | "OR" = "OR"): Promise<CardData[]> {
  const allData = await getPromptData(lang);
  ...
  return candidates.filter((card) => {
    const langData = card[safeLang] || card["en"] || card["zh-Hans"] || {};
